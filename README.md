...
現在は第7区間から第8区間への連続確認を行うため、Play時は`Main S06 Merge`上から第7区間として開始します。下ルート到達相当のヒモ残量39、選択長4で始まり、落下または`R`でも同じ地点へ戻ります。全区間確認へ戻す時は`MainStageRespawnOnFall`の開発用開始フラグを無効にします。
