...
- `E`を離す: 紐を回収し、使った長さの70%を戻す（30%は消費される）
...
`currentLength` が本作の「単一の真実」です。紐を出すと `TrySpend`、回収すると `Refund` が呼ばれます。通常解除では使った長さの70%だけを `Refund` し、残り30%が永久消費になります。落下してリスポーンすると最大値へ戻ります。
