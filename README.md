...
3. **ヒモを足場にする**：T2右岸の右端X=`30`と新しい右岸の左端X=`36`を使い、頭上に振り子用Hookがない谷幅6を作る。両岸の内側端には本編と同じ緑色の足場用フックを置く。谷の手前から対岸の緑フックへ長さ7で`E`接続し、`Q`で2つの緑フック間にたわみ1の足場を作る。残量20から7を永久消費して13になり、右岸上面へ着地した時だけT4の復帰地点を記録する
4. **Hookを外してまとめる**：資料T5を、本編に対応する内容がない旧T4の代わりに最終区間として繰り上げる。T3右岸の右端X=`42`から中央Hook、右岸の左端X=`52`までを、長さ6の足場2本で結ぶ。残量13から12を消費して残り1になる。2本のままでは吊り梁に当たるが、中央の青Hookへ照準を合わせて`F`で外すと、両端距離10・総長12・たわみ2の1本になり、梁の下を通ってゴールできる
...
現在はチュートリアル最終区間の検証のため、Play時は`Tutorial.unity`のT4左岸から、残量13・選択長6・T4の足場なしで開始します。長さ6の足場を2本作り、中央Hookを`F`で外して梁の下を通ります。落下や`R`での再挑戦も同じ状態へ戻ります。本編第10区間の開発開始設定はシーン内に残っているため、Play開始シーンを`MainStage.unity`へ戻せばフィナーレの検証も再開できます。
