...
1. **掛けて、振って、渡る**：下まで続く2つの地形を谷幅6で置き、中央上のHookへ長さ6固定で掛ける。プレイヤーの中心と当たり判定を含め、左右の岸端からHook中心まで約6になる高さへ補正しているため、掛けた瞬間は床に立ったままで、歩き出した時に振り子になる。T1中の照準は中央Hookへ自動で向き、未説明の矢印操作を要求しない。対岸への着地で接続を自動解除してT2へ進む。T1では画面にも`E`と`A/D`だけを出し、`W/S`と`Q`はまだ教えない。旧チュートリアル由来の後続Hook・床は各区間へ到達するまで非表示かつ当たり判定を無効にし、落下時は0.15秒で区間先頭へ戻る。ヒモは減らない
