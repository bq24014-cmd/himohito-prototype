...
3. **ヒモを足場にする**：T2右岸の右端X=`30`と新しい右岸の左端X=`36`を使い、頭上にHookがない谷幅6を作る。谷の手前から対岸の地形面へ長さ7で`E`接続し、`Q`で両岸の内側端へ正確に固定された、たわみ1の足場へ変える。残量20から7を永久消費して13になり、右岸上面へ着地した時だけT4の復帰地点を記録する
...
現在はチュートリアルT3制作のため、Play時は`Tutorial.unity`のT3左岸から、残量20・選択長7・足場なしで開始します。`R`での再挑戦も同じ状態へ戻ります。本編第10区間の開発開始設定はシーン内に残っているため、Play開始シーンを`MainStage.unity`へ戻せばフィナーレの検証も再開できます。
