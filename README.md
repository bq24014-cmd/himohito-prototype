...
現在は第9区間からフィナーレまでを繰り返し確認するため、Play時は`Main S07 Landing`上から第9区間として開始します。ヒモ残量27、選択長6で始まり、第9区間の2本に12、フィナーレに10を使うと残り5になります。落下または`R`でも現在の区間へ戻ります。全区間確認へ戻す時は`MainStageRespawnOnFall`の開発用開始フラグを無効にします。
