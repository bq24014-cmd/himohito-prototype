...
6. **配分の分かれ道**：第5区間右岸の端から分岐する。棚とフック群は開始床より上へ3離して配置する。上は緑Hookへ長さ7で接続して`Q`で安全橋を作り、3枚の棚をジャンプして合流床へ進む（消費7）。下は長さ6で青Hook3個を順に振り継ぎ、足場化せず合流床へ着地する（消費0）。合流チェックポイントの補充下限は32なので、上ルートの残量32と下ルートの残量39の差をそのまま保存する。途中で落下または`R`を押すと第6区間の開始地点へ戻る
