...
9. **外して、抜ける**：中央Hookを両岸より1.1高く置き、左岸から中央Hook、中央Hookから右岸へ長さ6の足場を2本作る。各区間の直線距離5.61に対してたわみは約0.39で、2本のまま歩くと低い梁に当たる。中央Hookへ照準を合わせて`F`で外すと、両端間11・総長12・たわみ1の1本へ統合され、足場の底が下がって梁の下を抜けられる
...
- 空中そのものには接続できない
- 生成したヒモ足場は通常時に歩ける。歩行中は足元の局所的な傾斜に沿って地上速度を補正し、たわみの底を越えた時の慣性で橋から浮かないようにする。ヒモ足場の上から`E`で接続した場合は、足場を離れるまで衝突を維持し、接続直後のすり抜けを防ぐ。離れた後は衝突を無効にし、振り子軌道を妨げない
...
- `Assets/Scenes/MainStage.unity`：第8区間を除き、第9区間まで実装済みの本編
...
- `HimoHito > Rebuild Main Stage Through Section 9`
...
現在は第9区間を繰り返し確認するため、Play時は`Main S07 Landing`上から第9区間として開始します。第7区間で長さ4の足場を作った後に相当するヒモ残量35、選択長6で始まり、落下または`R`でも同じ地点へ戻ります。全区間確認へ戻す時は`MainStageRespawnOnFall`の開発用開始フラグを無効にします。
