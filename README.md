...
現在はチュートリアルT3制作のため、Play時は`Tutorial.unity`のT3左岸から、残量20・選択長7・足場なしで開始します。対岸の緑フックへ接続して`Q`で橋を作ります。`R`での再挑戦も同じ状態へ戻ります。本編第10区間の開発開始設定はシーン内に残っているため、Play開始シーンを`MainStage.unity`へ戻せばフィナーレの検証も再開できます。
