...
接続できるフックを狙うと、既存の輪郭強調に薄い暖色の光と接続点の小さな目印を重ね、照準線の先端を実際の接続点に合わせます。青／緑リングそれぞれの絵の大きさに沿って約0.2秒で明るくなります。届かない場合は従来の破線リングと選択長の照準線を維持。E接続・照準外・ポーズ・失敗／ゴールなどの操作停止で光と目印を消します。第6区間の空中再接続は、既存処理が選ぶ次のフックを示します。矢印キーの照準角・射程・接続判定・ジョイント・資源・音・カメラは変えません。
...
落下で失敗が確定すると、頭側から編み目がほどけ、同じ毛糸素材の6本の短いヒモへ変わる演出を約0.8秒表示してから失敗画面へ移ります。落下時の姿と位置をそのまま使い、演出のために画面内へ引き戻しません。画面外で失敗した場合、ほどける演出も画面外で進みます。主人公本体・カメラ・落下判定位置は動かしません。演出中は操作を止め、終了後はRで再挑戦／Escでタイトルに戻るまで失敗画面を維持します。再挑戦では元の描画を戻し、0.8秒の編み戻りへつなぎます。ヒモ切れの失敗ではこの落下演出を再生せず、追加の音・フラッシュ・カメラ揺れはありません。
...
本編の区間到達は、復帰地点を持つゴール床の上面に着地したときだけ確定します。壁・床の裏・中継床への接触では次区間へ進めず、復帰地点や残量補充、到達の毛糸旗も更新しません。フック接続中は従来どおり確定せず、解除して上面に乗ると確定します。

ステージ開始時に宝箱からスタート地点へカメラが戻る見渡し演出を再生します。チュートリアルではタイトルの「はじめる」を押した後、本編では読み込み後に開始。宝箱で0.7秒止まり、同じ高さ・倍率のまま最大12ユニット／秒で左へ進み、最初と最後の約1秒だけ穏やかに加減速します。目安はチュートリアル約7.5秒／本編約20秒。`Space`／`Enter`でスキップでき、スタートへ戻った後に操作を再開します。見渡し中は移動・ヒモ操作・R再挑戦・Tab／Esc／Zと通常HUDを止めます。プレイヤーと復帰地点は移動せず、ヒモ残量・選択長・ステージ配置も変更しません。通常のR再挑戦では見渡しを繰り返しません。
...
- 看板を見る仕草：チュートリアルの現在区間の看板近くで、地上で無操作のまま約1.6秒待つと、既存の視線ポーズで約1.15秒だけ看板を見ます。1回の接近につき1度だけで、キー操作・移動・ジャンプ・接続で中断します。照準線そのものは動かさず、Zによる開閉も自動では行いません。

ポーズ・操作説明・看板を開いている間は、ほこり・床の反応・看板を見る仕草の時間を進めません。

本編／チュートリアルの部屋背景では、中央5枚の旗と壁の3つの星に、約4〜6秒周期の揺れを加えています。既存PNGは変更せず、飾り周辺の描画座標だけを局所的に変化させる方式です。最初の設定では変化が分かりにくかったため、星は約4倍、旗は約2.7倍の振幅に調整しました。通常の画面では旗が上端の外にあることが多いため、左寄りの青・赤・茶色の星を見ると確認しやすくなっています。背景全体・家具・カメラ・当たり判定は揺らさず、タイトルなどの画面背景と遠景は対象外。ポーズ中は止まり、再開すると同じ位置から続きます。

木製足場は既存の木目画像に薄い上面・縁・側面と下端の陰影を重ね、乗れる面と厚みを示します。表示専用で、足場の位置・サイズ・コライダーは変更しません。主人公の真下に木製足場があると小さな柔らかい接地影を表示し、高さに応じて薄く、約1.9ユニット離れると消します。谷やヒモ橋の上では表示せず、床端からもはみ出しません。チュートリアル／本編共通で、Sceneビューの足場表示は既存のRefresh Scene Visualsから更新できます。
...
ヒモ橋に立つと、足元から約0.85ユニット以内の毛糸だけが約0.12秒で軽く押され、最大14％細く締まります。静止中も小さな沈み（最大0.025）を保ち、足を離すと約0.3秒で太さと形が戻ります。歩行時の足音に合わせた約0.34秒の沈みも残し、合計の沈みは最大0.04に制限します。橋の端・コライダー・プレイヤーを支える曲線は固定で、Q生成／F統合演出中は足元の反応を出しません。ポーズ中は止まり、音の追加はありません。
...
ステージの棘は、積み木に合わせた赤い塗装・木目・木の縁のある三角形で表示します。チュートリアル・本編共通で、表示のみ差し替え、接触判定とヒモの解除条件は従来どおりです。

チュートリアル最終区間のゴール床はX=52〜67（幅15）。左端のフックと橋の距離は維持し、宝箱はX=63に配置しています。渡り終わってから宝箱へ歩く余白を確保しています。
本編のゴール床も幅15（X=205〜220）、宝箱X=216とし、両ステージとも床の左端から宝箱中心まで11、右端まで4の配置に統一しています。床の高さ・橋の両端は従来どおりです。

本編第4区間の着地床（幅6）はX=83.1〜89.1へ右2移動し、そこから最終ゴールまでの床・フック・レール・照明・チェックポイントも一緒に右2移動しています。第4区間の中継柱と青Hookは据え置き、第5区間以降の相対距離・高さ・足場サイズ・ヒモ消費は維持します。
...
3. **二段Hook**：上下2つのHookから、長さと着地点を考える。上Hookは長さ8で左端X=46.6の高い棚へ進む。下Hookは開始床より2.7高い位置（X=38.2、Y=-1.65）にあり、長さ6で低い行き止まりへ着地する。床の玩具スイッチを`F`で押すと帰りの段差が出て、開始床へ戻ると消える
4. **届かない対岸**：第3区間の棚右端から遠い青Hookへ直接長さ13で掛けると、最下点Y=-10.45まで落ちて対岸へ届かない。左岸と柱上の緑色の足場用Hookは対になっており、両端は横3.7・上1.5（直線距離約3.99）離れている。右の緑Hookへ長さ5で接続して`Q`を押すと、通常ジャンプでは届かない中継柱へ歩ける足場ができる。柱上から`(72.6,2.55)`の青Hookへ長さ9で掛けて対岸へ渡る。緑色は2点を結ぶ橋作成用、青は振り子用を示し、足場化で残量を50から45へ減らす最初の本編区間
   手前の床は左端X=46.6を固定し、右端だけ61.6まで1延長（幅15）。緑Hookも右端へ移動。中継床の左端付近からA＋Spaceで手前の床へ戻れるようにし、ジャンプ力・復帰位置・後続区間の配置は変更しません。
5. **懐中電灯を遮る**：第4区間の壁床右端`X=89.1`と左端を揃えた青いレール棚へ進み、棚の内側にある2つの緑Hookを足場で結ぶ。棚間4.4へ届く長さなら生成でき、推奨の長さ6ではたわみ1.6になって光源の真下を遮光する。光が0.4秒で暗くなったら、谷側の青Hook`(99.0, 2.95)`へ長さ7で掛けて右岸へ渡る。棚2枚は未接続時と緑の橋作成Hookへの接続中には上に立てるが、青い振り子Hookへの接続中は軌道を妨げないようプレイヤーとの衝突がなくなる。`R`または落下で再挑戦した時は、2枚とも本体・当たり判定・レール画像を即座に復元する。右棚端から右岸までは約8.6空け、通常ジャンプの近道を防ぐ。茶色い取付ブロックは青Hookの真上に揃え、遮光判定用の見えない起点だけを橋中央に残す。旧構成の第7区間Hookは第5区間内に重なるため非表示にする
