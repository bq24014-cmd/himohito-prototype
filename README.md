...
- `←` / `→`: 照準線を滑らかに回転する
- `↑` / `↓`: 照準線を真上／真下へ素早く合わせる
- `E`を押し続ける: 狙った方向へ紐を射出・固定
- `E`を離す: 紐を回収し、使った長さを戻す
...
- 地上の最高速度は `12` とし、開始地点での移動が遅く感じないようにする
...
開始地点、黄色の接続地点、安全な着地点、難しい着地点、カメラを自動配置するEditor専用コードです。ゲーム中の挙動ではなく「検証環境を再現するコード」なので、最後に読めば十分です。
