...
6. **配分の分かれ道**：第5区間右岸の端から分岐する。上は緑Hookへ長さ5で接続して`Q`で安全橋を作り、3枚の棚をジャンプして合流床へ進む（消費5）。下は長さ4で青Hook3個を順に振り継ぎ、足場化せず合流床へ着地する（消費0）。合流チェックポイントの補充下限は34なので、上ルートの残量34と下ルートの残量39の差をそのまま保存する。途中で落下または`R`を押すと第6区間の開始地点へ戻る
