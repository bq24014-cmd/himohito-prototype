...
現在はフィナーレを繰り返し確認するため、Play時は`Main S09 Goal Floor`上から第10区間として開始します。ヒモ残量15、選択長10で始まり、最後の橋に10を使うと残り5になります。落下または`R`でも第10区間の入口へ戻ります。全区間確認へ戻す時は`MainStageRespawnOnFall`の開発用開始フラグを無効にします。
