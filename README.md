...
8. **垂れた底を道にする**：第7区間の右岸端と、5離れた対岸の緑Hookをヒモ足場で結ぶ。現在の足場式では中央の底が`端の高さ - (選択長 - 5)`になるため、長さ7は深さ2、長さ8は深さ3で安全に穴へ降りられる。長さ5〜6は浅く、右の木製ゲートが開かない。長さ9以上は足場自体が底のトゲへ触れて区間の最初へ戻る。長さ7〜8の足場を作った時だけゲートが開き、たるみの底から横約1.6の低い出口へジャンプして抜ける。急な右斜面を最後まで登る必要はない
