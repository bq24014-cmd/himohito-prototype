...
ヒモが尽きた時は、床に残った短いヒモと遠いおもちゃ箱を描いた共通の失敗画面を表示します。`R`で現在区間から再挑戦し、`Esc`でタイトルへ戻れます。落下時の短い自動復帰は従来どおりです。
現在はPart Dの確認用に、Playすると本編の失敗画面から開始します。そこで`R`を押すと、通常の残量50で本編第1区間へ戻ります。そのまま横へ進むと、追加した遠景レイヤーの視差を同じPlay中に確認できます。
...
1920×1080のウィンドウモードで起動します。スプラッシュは濃紺背景に主人公ロゴと
Unityロゴを順番に表示します。Unity 6では従来の起動時解像度ダイアログは使わず、
視認性と操作の安定を優先した固定ウィンドウで起動します。
