...
8. **垂れた底を道にする**：第7区間の右岸端と、5離れた対岸の緑Hookをヒモ足場で結ぶ。左右の壁から1ずつ張り出した床により、縦穴の口は幅3に見える。張り出しはプレイヤーの後ろ角が引っ掛からない非衝突の目印として扱う。現在の足場式では中央の底が`端の高さ - (選択長 - 5)`になるため、長さ7は深さ2、長さ8は深さ3で穴の口へ安全に降りられる。長さ5〜6は穴の口より浅く、長さ9以上は足場自体が底のトゲへ触れて区間の最初へ戻る。安全な足場の底から横約1.6の低い通路へジャンプして抜ける
...
- 空中そのものには接続できない
- 生成したヒモ足場は通常時に歩ける。歩行中は足元の局所的な傾斜に沿って地上速度を補正し、たわみの底を越えた時の慣性で橋から浮かないようにする。振り子中に自分の足場が軌道を妨げないよう、接続中だけプレイヤーとの衝突を無効にする
- 赤い木製玩具の三角トゲや懐中電灯へ接続中のプレイヤーが触れると、ヒモが外れて落下する。第8区間の穴底では、プレイヤーまたは長すぎる生成足場がトゲに触れると現在の区間から再挑戦になる
...
- `Assets/Scenes/MainStage.unity`：第8区間まで実装済みの本編
...
- `HimoHito > Rebuild Main Stage Through Section 8`
...
現在は第8区間を繰り返し確認するため、Play時は`Main S07 Landing`上から第8区間として開始します。第7区間で長さ4の足場を作った後に相当するヒモ残量35、選択長7で始まり、落下または`R`でも同じ地点へ戻ります。全区間確認へ戻す時は`MainStageRespawnOnFall`の開発用開始フラグを無効にします。
